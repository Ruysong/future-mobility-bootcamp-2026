...
차량용 내부 통신 네트워크에서 Ethernet을 사용하는 배경과 RA6M3/RZN 보드 기반 Ethernet 실습을 정리한 과목 폴더입니다. 1일차는 Automotive Ethernet, MAC/PHY, VLAN backbone 실습을 다루고, 2일차는 TSN, PTP 시간 동기화, TAS(Time-Aware Shaper) 실습까지 이어집니다.
...
| 1일차 원본 교안 | [이더넷_강의자료.pdf](<교안/이더넷_강의자료.pdf>) | 152페이지 차량용 Ethernet 1일차 실습자료 |
| 1일차 텍스트 정리 | [이더넷_강의자료.md](<텍스트/이더넷_강의자료.md>) | 이론, Hardware Configuration, 실습 A1/A2/A3 정리 |
| 2일차 원본 교안 | [이더넷_강의자료(2일차).pdf](<교안/이더넷_강의자료(2일차).pdf>) | 300페이지 TSN/PTP/TAS 실습자료 |
| 2일차 텍스트 정리 | [이더넷_강의자료(2일차).md](<텍스트/이더넷_강의자료(2일차).md>) | TSN 표준, PTP 시간 동기화, TAS 적용/미적용 실습 정리 |
...
| Background C | TSN 필요성과 표준 | IEEE 802.1AS, 802.1CB, 802.1Qbv, 802.1Qch, 802.1Qcr |
| PTP 실습 | Master/Slave 시간 동기화 | Sync, Follow_Up, Delay_Req, Delay_Resp, Wireshark |
| TAS 실습 | Time-Aware Shaper 적용/미적용 비교 | GCL, Gate, Guard Band, ST/MM/BE traffic |
...
- TSN은 Ethernet 기반 차량 네트워크에서 시간 동기화, 지연 보장, traffic scheduling, frame redundancy를 제공하는 IEEE 표준 묶음이다.
- PTP/IEEE 802.1AS는 Master/Slave clock을 맞춰 TAS처럼 시간표 기반으로 동작하는 traffic 제어가 가능하게 한다.
- TAS(IEEE 802.1Qbv)는 Gate Control List로 queue gate를 열고 닫아 Scheduled Traffic을 보호하고, Guard Band로 다음 slot 침범을 방지한다.
...
- PTP packet을 Wireshark로 확인하며 Sync, Follow_Up, Delay_Req, Delay_Resp 기반 시간 동기화 흐름을 분석했다.
- TAS 실습에서 ST/MM/BE traffic을 나누고, PTP ON 상태에서 GCL 기반 scheduling이 지연과 traffic 간섭을 줄이는 과정을 확인했다.
