...
| 원본 교안 | [DDS 단기강좌 강의자료.pdf](<교안/DDS 단기강좌 강의자료.pdf>) | 170페이지 DDS 2일차 실습자료 |
| 텍스트 정리 | [단기강좌_SOMEIP_교육자료.md](<텍스트/단기강좌_SOMEIP_교육자료.md>) | Wireshark 설정, PC-PC 실습, PC-Board 실습 정리 |
| 텍스트 정리 | [DDS 단기강좌 강의자료.md](<텍스트/DDS 단기강좌 강의자료.md>) | DDS 이론, Ubuntu 환경설정, Fast DDS 실습 정리 |

## 무엇을 했는가

차량용 미들웨어 과목은 SOME/IP와 DDS를 나누어 실습했습니다. SOME/IP는 서비스 호출과 SD 기반 구독 흐름을 패킷 단위로 확인했고, DDS는 데이터 중심 Pub/Sub와 QoS 정책을 Linux/Fast DDS 환경에서 확인했습니다.
...
| 실습 | 주제 | 핵심 내용 | 확인 포인트 |
| --- | --- | --- | --- |
| 0 | Basic Counter | IDL, fastddsgen, Publisher/Subscriber | 기본 Pub/Sub 메시지 송수신 |
| 1 | Domain | Domain ID 기반 논리적 격리 | 같은 Domain에서만 통신 |
| 2 | Topic | Topic Name/Type/Key/Instance | Vehicle Diagnostics 단일 Topic |
| 3 | Multi Topic | Powertrain, Chassis, Battery, ADAS | Topic 구독/해지 |
| 4 | QoS Reliability | Reliable vs Best Effort | Critical message 재전송, missing sequence |
| 5 | QoS History | KEEP_LAST vs KEEP_ALL | 저장 Sample 개수 차이 |
| 6 | QoS Ownership | Ownership Strength | 다중 Publisher 우선순위 |
...
- DDS는 OMG 표준 데이터 중심 Pub/Sub 미들웨어이며, Broker 없이 Peer-to-Peer로 Topic 데이터를 교환한다.
- DCPS는 애플리케이션 레벨 API와 데이터 중심 모델을 제공하고, RTPS는 UDP/IP 기반 전송과 DDS 구현체 간 상호운용성을 담당한다.
- DDS QoS는 Reliability, History, Ownership 같은 정책으로 신뢰성, 저장 방식, 다중 Publisher 우선순위를 제어한다.
...
- DDS 실습에서는 Domain, Topic, Key/Instance, QoS 정책을 직접 실행하며 데이터 중심 Pub/Sub 구조를 확인했다.
- Wireshark에서 SOME/IP는 `someip || someipsd`, DDS는 `rtps` 필터를 사용해 각각의 하위 패킷 구조를 확인했다.
